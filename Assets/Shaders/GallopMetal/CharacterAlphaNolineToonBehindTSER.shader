// Metal re-authoring of Gallop/3D/Chara/AlphaNolineToonBehind/TSER (URP). Same shared
// GallopMetalToon core as AlphaNolineToon/TSER; the in-game "Behind" variant is the transparent
// pass drawn behind the body, so it renders back faces and doesn't write depth.
Shader "Gallop/3D/Chara/AlphaNolineToonBehind/TSER"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(VaryToon IN) : SV_Target { return ToonShade(IN, true); }
            ENDHLSL
        }
    }
    Fallback Off
