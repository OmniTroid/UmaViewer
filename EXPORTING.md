...
not, loops never do. When a motion has a `_cam`, the export records it as a camera VMD of its own
next to the motion file, `<motion>_camera.vmd` with the model name カメラ・照明, the way MMD camera
motions are distributed (`--no-camera` skips it). A card cut-in (`anm_cti_crd..._01`) is one cut of a chain the
